...
//        this(context, attrs, R.style.PasswordInputViewStyle);
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        TypedArray t = context.obtainStyledAttributes(attrs, R.styleable.VerificationCodeView);

        maxLength = t.getInt(R.styleable.VerificationCodeView_pwv_maxLength, 4);
        borderColor = t.getColor(R.styleable.VerificationCodeView_pwv_borderColor, Color.GRAY);
        pwdColor = t.getColor(R.styleable.VerificationCodeView_pwv_pwdColor, Color.GRAY);
        inputBorderColor = t.getColor(R.styleable.VerificationCodeView_pwv_haveInputBorderColor, borderColor);
        asterisk = t.getString(R.styleable.VerificationCodeView_pwv_asterisk);

        if (asterisk == null || asterisk.length() == 0) asterisk = "*";
        else if (asterisk.length() > 1) this.asterisk = asterisk.substring(0, 1);

        radius = t.getDimensionPixelSize(R.styleable.VerificationCodeView_pwv_radius, 0);
        strokeWidth = t.getDimensionPixelSize(R.styleable.VerificationCodeView_pwv_strokeWidth, 2);
        spacing = t.getDimensionPixelSize(R.styleable.VerificationCodeView_pwv_spacing, 0);
        borderStyle = t.getInt(R.styleable.VerificationCodeView_pwv_borderStyle, BorderStyle.BOX);
        pwdStyle = t.getInt(R.styleable.VerificationCodeView_pwv_pwdStyle, PwdStyle.CIRCLE);
